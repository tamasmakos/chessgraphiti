...
export interface GameStore {
  ...
  startAnalysis: () => void;
  exitAnalysis: () => void;
  navigateAnalysis: (direction: "first" | "prev" | "next" | "last" | number) => void;
...
function buildAnalysisState(history: MoveRecord[], targetIndex?: number) {
  ...
  const analysisIndex = Math.max(0, Math.min(lastIndex, targetIndex ?? lastIndex));
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  makeMove: (from, to, promotion) => {
    ...
    let gameStatus: GameStatus = state.gameStatus === "idle" ? "playing" : state.gameStatus;
    ...
    const graphSnapshot = state.liveGraphEnabled ? computeGraph(game.fen()) : state.graphSnapshot;

    const nextLiveSnapshots = state.liveGraphEnabled
      ? [...state.liveGraphSnapshots, graphSnapshot]
      : [];

    const nextLiveLineage =
      state.liveGraphEnabled && graphSnapshot
        ? computeNextStepLineage(graphSnapshot, state.graphSnapshot, state.liveLineage)
        : state.liveLineage;
  ...
  undo: () => {
    ...
    const nextLiveLineage =
      state.liveLineage && state.liveGraphEnabled
        ? {
  ...
  navigateAnalysis: (direction) => {
    ...
    if (state.gameStatus !== "analysis") {
      ...
      const targetIndex = resolveAnalysisIndex(direction, liveLastIndex, liveLastIndex);
    ...
    const shouldResumeLivePlay = newIndex === lastIndex && state.gameOverReason === undefined;
  ...
  setEngineStrength: (engineStrength) => set({ engineStrength }),
  ...
  setTutorMode: (on) =>
    set({
      tutorMode: on,
      tutorRanking: on ? get().tutorRanking : null,
      tutorWinProb: on ? get().tutorWinProb : undefined,
    }),

  setTutorData: (ranking, winProb) =>
    set({ tutorRanking: ranking, tutorWinProb: winProb, isTutorAnalyzing: false }),
