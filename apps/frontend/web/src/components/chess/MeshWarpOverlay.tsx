  ...
  const polygons = useMemo(() => {
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        ...
        const xElev =
          ((grid.total[row * 9 + xEdgeCol] ?? 0) + (grid.total[(row + 1) * 9 + xEdgeCol] ?? 0)) / 2;
        const xW =
          ((grid.white[row * 9 + xEdgeCol] ?? 0) + (grid.white[(row + 1) * 9 + xEdgeCol] ?? 0)) / 2;
        const xB =
          ((grid.black[row * 9 + xEdgeCol] ?? 0) + (grid.black[(row + 1) * 9 + xEdgeCol] ?? 0)) / 2;

        // Bottom-edge (south) elevation for the south wall face
        const bottomElev =
          ((grid.total[(row + 1) * 9 + col] ?? 0) + (grid.total[(row + 1) * 9 + col + 1] ?? 0)) / 2;
        const bottomW =
          ((grid.white[(row + 1) * 9 + col] ?? 0) + (grid.white[(row + 1) * 9 + col + 1] ?? 0)) / 2;
        const bottomB =
          ((grid.black[(row + 1) * 9 + col] ?? 0) + (grid.black[(row + 1) * 9 + col + 1] ?? 0)) / 2;
