...
function processSnapshot(snap: GraphSnapshot): PerStepMetric {
  ...
  for (const node of snap.nodes) {
    ...
    if (!communityMix.has(node.communityId))
      communityMix.set(node.communityId, { white: 0, black: 0 });
    const bucket = communityMix.get(node.communityId)!;
    if (node.color === "w") bucket.white++;
    else bucket.black++;
  }
...
function computeImportanceMap(
  ...
  for (let si = 0; si < numSteps; si++) {
    ...
    for (const n of snap.nodes) {
      ...
      arr.push(
        (n.centralityWeighted +
          n.centralityDegree +
          n.centralityBetweenness +
          n.centralityCloseness +
          n.centralityPageRank) /
          5,
      );
...
function computeAttackNorms(perStepMetrics: Map<number, PerStepMetric>) {
  ...
  let maxIn = 0.0001;
  for (const v of attackInByStepCid.values()) {
    if (v > maxIn) maxIn = v;
  }
  ...
  if (si === 0) {
    ...
    if (firstT) {
      for (const link of firstT.links)
        counts.set(
          link.fromCommunityId,
          (counts.get(link.fromCommunityId) ?? 0) + link.overlapWeight,
        );
    } else {
...
function buildStepNodes(p: BuildStepNodesParams): {
  nodes: RichNode[];
  geometry: Map<number, RichNode>;
} {
...
function buildFlowLinksForStep(p: BuildFlowLinksParams): FlowLink[] {
  ...
  const sortedLinks = [...transition.links].sort((a, b) => {
    const ay =
      (p.nodeGeometry[p.ri - 1]?.get(a.fromCommunityId)?.y ?? 0) +
      (p.nodeGeometry[p.ri]?.get(a.toCommunityId)?.y ?? 0);
    const by =
      (p.nodeGeometry[p.ri - 1]?.get(b.fromCommunityId)?.y ?? 0) +
      (p.nodeGeometry[p.ri]?.get(b.toCommunityId)?.y ?? 0);
    return ay - by;
  });
...
export function CommunityLineageGraph({
  ...
  const { steps, nodes, links, sparklineData } = useMemo(() => {
    const empty = {
      ...
      nodes: [] as RichNode[],
      links: [] as FlowLink[],
      sparklineData: [] as { step: number; value: number }[],
    };
    if (!analysis?.stableColorByStep || numTotalSteps === 0 || windowedSteps.length === 0)
      return empty;

    const importanceMap = computeImportanceMap(analysisGraphSnapshots, numTotalSteps);
    let globalMaxImp = 0;
    for (const m of importanceMap.values())
      for (const v of m.values()) {
        if (v > globalMaxImp) globalMaxImp = v;
      }
    if (globalMaxImp < 0.0001) globalMaxImp = 1;

    const { maxCommunityAttackOut, maxCommunityAttackIn, attackInByStepCid } =
      computeAttackNorms(perStepMetrics);
    ...
    for (let ri = 0; ri < numWindowed; ri++) {
      ...
      const { nodes: stepNodes, geometry } = buildStepNodes({
        si,
        ri,
        stepMap,
        analysis,
        ...
        perStepMetrics,
        attackInByStepCid,
        maxCommunityAttackIn,
        maxCommunityAttackOut,
        chartHeight,
        stepGap,
      });
    ...
    const sparklineData = windowedSteps.map((si) => ({
      step: si,
      value: perStepMetrics.get(si)?.totalCrossAttack ?? 0,
    }));
  ...
  return (
    <div className="w-full space-y-1" ref={containerRef}>
      ...
      <div className="flex justify-between items-center px-1">
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-glow-indigo">
          Molecular Evolution
        </span>
      ...
      >
        <svg ref={svgRef} width="100%" height="100%" className="overflow-visible">
          ...
          {steps.map((s, ri) => (
            <line
              key={`grid-${s}`}
              x1={PADDING.left + ri * stepGap}
              y1={PADDING.top}
              x2={PADDING.left + ri * stepGap}
              y2={chartBottom}
              ...
              return (
                <g
                  ...
                  onMouseEnter={(e) => {
                    ...
                    if (!svgRect) return;
                    setTooltip({
                      x: node.x + svgRect.left + NODE_WIDTH + 6,
                      y: node.y + svgRect.top + node.h / 2,
                      communityId: node.id,
                      step: node.step,
                      weight: node.weight,
                      whiteFraction: node.whiteFraction,
                      attackIn: node.attackPressureIn,
                      attackOut: node.attackPressureOut,
                      color: node.color,
                    });
                  }}
                ...
                >
                  {/* Main community rect */}
                  <rect
                    x={node.x + MIXTURE_STRIP}
                    y={node.y}
                    width={node.w - MIXTURE_STRIP}
                    height={node.h}
                    fill={node.color}
                    rx={2}
                    style={{
                  ...
                  {node.h >= MIN_NODE_HEIGHT * 2 && (
                    <>
                      <rect
                        ...
                        y={node.y}
                        width={MIXTURE_STRIP}
                        height={Math.max(1, node.h * node.whiteFraction)}
                        fill="rgba(255,255,255,0.75)"
                        rx={1}
                      />
                      <rect
                        x={node.x}
                        y={node.y + node.h * node.whiteFraction}
                        width={MIXTURE_STRIP}
                        height={Math.max(1, node.h * (1 - node.whiteFraction))}
          ...
          {steps
            ...
            .filter(
              ({ ri }) =>
                ri % Math.max(1, Math.floor(numWindowed / 7)) === 0 || ri === numWindowed - 1,
            )
            .map(({ s, ri }) => (
              <text
                key={`label-${s}`}
                x={PADDING.left + ri * stepGap}
                y={xLabelY}
                textAnchor="middle"
                style={{
                  fontSize: "9px",
                  fill: s === currentIndex ? "#818cf8" : "#475569",
          ...
          {sparklineData.length > 1 && (
            <g>
              <text
                x={PADDING.left - 4}
                y={sparklineTopY + SPARKLINE_HEIGHT / 2 + 3}
                textAnchor="end"
                style={{ fontSize: "7px", fill: "#475569", fontFamily: "monospace" }}
              >
                atk
              </text>
      ...
      <div className="flex justify-between items-center text-[9px] text-slate-500 font-mono px-1">
        <span className="flex items-center gap-2.5">
          <span className="flex items-center gap-1">
            <span
              ...
              style={{
                background:
                  "linear-gradient(to bottom, rgba(255,255,255,0.8) 50%, rgba(15,23,42,0.9) 50%)",
              }}
            />
            <span>side mix</span>
          </span>
        ...
        <span className="text-indigo-400 opacity-60 italic">
        ...
        >
          <div className="flex items-center gap-1.5 mb-1.5">
            <span
              ...
              style={{ background: withAlpha(tooltip.color, 0.9) }}
            />
            <span className="text-[9px] font-bold text-slate-200 uppercase font-mono">
              Community {tooltip.communityId}
            </span>
          </div>
          <div className="space-y-0.5 text-[9px] text-slate-400 font-mono">
            <div>
              Move <span className="text-indigo-300">{tooltip.step}</span>
            </div>
            <div>
              Pieces <span className="text-slate-200">{tooltip.weight}</span>
            </div>
