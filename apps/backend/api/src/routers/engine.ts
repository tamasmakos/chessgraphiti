...
const mapEngineError = (
  error: Error,
  errors: {
    BAD_REQUEST: (o: { message: string }) => Error;
    INTERNAL_SERVER_ERROR: (o?: { message: string }) => Error;
  },
  ...
  if (error instanceof EngineTimeoutError)
    throw errors.BAD_REQUEST({ message: "Engine timed out" });
  if (error instanceof EngineExecutionError)
    throw errors.INTERNAL_SERVER_ERROR({ message: error.message });
...
export const engineRouter = () => {
  ...
  return orpc.router({
    listFiles: orpc.handler(async ({ errors }) => {
    ...
    bestMove: orpc.input(getBestMoveInputSchema).handler(async ({ input, errors }) => {
    ...
    analyzePosition: orpc.input(analyzePositionInputSchema).handler(async ({ input, errors }) => {
