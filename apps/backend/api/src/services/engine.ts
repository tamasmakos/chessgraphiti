...
export const getBestMoveInputSchema = z.object({
  ...
  modelPath: z
    .string()
    .min(1)
    .refine((v) => v === path.basename(v), "Path traversal not allowed"),
  bookPath: z
    .string()
    .refine((v) => v === path.basename(v), "Path traversal not allowed")
    .optional(),
...
export class EngineService {
  ...
  private queue: Promise<Result<EngineBestMoveResult, Error>> = Promise.resolve(
    ok({ bestMove: "", score: 0, depth: 0, pv: [], nodes: 0 }),
  );
  ...
  private spawnEngine(
    ...
    const proc = spawn("python", args, {
      ...
      cwd: path.dirname(this.engineScript),
    });
  ...
  private async ensureEngine(
    ...
    if (
      this.engine?.state.phase !== "starting" &&
      this.engine?.modelPath === modelPath &&
      this.engine?.bookPath === bookPath
    ) {
      return ok(undefined);
    }
...
function parseBestMoveResult(
  line: string,
  engine: PersistentProc,
): Result<EngineBestMoveResult, Error> {
